...
public class StatefulObjectInspector : UnityEditor.Editor
{
    ...
    private void OnEnable()
    {
        ...
        _setDefaultStateOnBadStateRequestProp = serializedObject.FindProperty("_setDefaultStateOnBadStateRequest");
        ...
        _reorderableList =
            new ReorderableList(serializedObject, serializedObject.FindProperty("_stateEntries"), true, true, true,
                true)
            {
    ...
    private void DrawListElement(Rect rect, int index, bool isActive, bool isFocused)
    {
        ...
        Rect stateNameArea = new Rect(rect) { width = rect.width * 0.333f };

        item.StateName = EditorGUI.TextField(stateNameArea, item.StateName);
        int padding = 4;
        Rect objectReferenceArea = new Rect(rect)
        {
            x = stateNameArea.x + stateNameArea.width + padding,
            width = rect.width - stateNameArea.width - padding
        };

        item.StateObject =
            EditorGUI.ObjectField(objectReferenceArea, item.StateObject, typeof(GameObject), true) as GameObject;
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_defaultStateName"));
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (currentStateIndex != -1)
                {
                    if (GUILayout.Button("<", EditorStyles.miniButtonLeft))
                    {
                        ...
                        statefulObject.TrySetState(statefulObject.GetStateFromIndex(currentStateIndex).StateName);
                    }
        ...
        {
            if (statefulObject.StateEntries[i] != null &&
                !string.IsNullOrEmpty(statefulObject.StateEntries[i].StateName))
            {
                if (statefulObject.StateEntries[i].StateObject != null &&
                    statefulObject.CurrentState.StateObject != null)
                {
                    if (statefulObject.StateEntries[i].StateObject.GetInstanceID() ==
                        statefulObject.CurrentState.StateObject.GetInstanceID())
                    {
